...
// CORRECCIÓN: Usamos ruta absoluta para evitar errores de resolución
    ...
    useEffect(() => {
        const loadDashboard = async () => {
            try {
                ...
                // Buscar la próxima cita pendiente
                const now = new Date();
                const upcoming = appointments
                    .filter(a => a.estado !== 'Finalizada' && a.estado !== 'Cancelada')
                    .sort((a, b) => new Date(a.fecha_hora) - new Date(b.fecha_hora))
                    .find(a => new Date(a.fecha_hora) > now) || null;
        ...
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex gap-3">
                    <button onClick={() => navigate('/dashboard/agenda')} className="bg-white border border-gray-200 text-gray-700 px-4 py-2 rounded-lg font-bold shadow-sm hover:bg-gray-50 flex items-center">
                        <CalendarDaysIcon className="w-5 h-5 mr-2 text-gray-400" /> Ver Agenda
                    </button>
                    <button onClick={() => navigate('/dashboard/pacientes')} className="bg-primary text-white px-4 py-2 rounded-lg font-bold shadow hover:bg-teal-700 flex items-center">
                    ...
                    <div className="bg-gradient-to-r from-slate-800 to-slate-900 rounded-2xl p-6 text-white shadow-lg flex justify-between items-center relative overflow-hidden">
                        <div className="relative z-10">
                            ...
                            {stats.nextAppointment ? (
                                <>
                                    <h2 className="text-2xl font-bold">{new Date(stats.nextAppointment.fecha_hora).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} - Paciente ID #{stats.nextAppointment.patient_id}</h2>
